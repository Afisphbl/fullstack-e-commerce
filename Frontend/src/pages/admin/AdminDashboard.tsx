  ...
  if (loading || !data) {
    return (
      <div className="flex min-h-[400px] items-center justify-center">
        <LoadingSpinner size="lg" label={t("admin:dashboard.loading")} />
      </div>
    );
  }
          ...
          >
            <div className="flex justify-between items-start mb-4">
              ...
              <button
                ...
                aria-label={t("admin:common.moreOptions")}
              >
              ...
              <div>
                ...
                >
                  ...
                  <span className="text-muted-foreground font-normal ml-1">
                    {t("admin:dashboard.inTheLastMonth")}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-6">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-medium text-muted-foreground">
                  {t("admin:dashboard.currentTrend")}
                </span>
              </div>
            </div>
          </div>
              ...
              <thead>
                <tr className="text-[11px] uppercase tracking-wider text-muted-foreground font-semibold border-b border-border">
                  <th className="px-6 py-4">
                    {t("admin:dashboard.productName")}
                  </th>
                  <th className="px-6 py-4">{t("admin:dashboard.price")}</th>
                  <th className="px-6 py-4">{t("admin:dashboard.category")}</th>
                  <th className="px-6 py-4 text-center">
                    {t("admin:dashboard.quantitySold")}
                  </th>
                  <th className="px-6 py-4">
                    {t("admin:dashboard.totalAmount")}
                  </th>
                ...
                {data.topProducts.slice(0, 5).map((product: Product) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-right">
                      <button
                        className="text-muted-foreground hover:text-foreground"
                        aria-label={t("admin:common.moreActions")}
                      >
                        <MoreHorizontal className="h-4 w-4" />
                      </button>
                    </td>
        ...
        <div className="lg:col-span-4 bg-card p-6 rounded-2xl shadow-sm border border-border flex flex-col justify-between">
          <div>
            <div className="flex justify-between items-start mb-1">
              <h3 className="font-display font-bold text-foreground">
                {t("admin:dashboard.currentPerformance")}
              </h3>
          ...
          <div className="py-8 flex flex-col items-center">
            ...
            <p className="mt-8 text-[11px] text-muted-foreground text-center px-4">
              {t("admin:dashboard.revenueChanged", {
                percent: data.stats?.revenueGrowth || 0,
              })}
            </p>
          </div>
        ...
        data.wishlistAnalytics.topWishlistedProducts.length > 0 && (
          <div className="bg-card rounded-2xl shadow-sm border border-border overflow-hidden">
            <div className="p-6 flex items-center justify-between border-b border-border">
              <div>
                <h3 className="font-display font-bold text-foreground">
                  {t("admin:dashboard.mostWishlisted")}
                </h3>
                <p className="text-xs text-muted-foreground mt-1">
                  {t("admin:dashboard.wishlistSubtitle")} •{" "}
                  {t("admin:dashboard.totalWishlistItems", {
                    count: data.wishlistAnalytics.stats.totalWishlistItems,
                  })}
                </p>
              </div>
              <div className="flex items-center gap-4">
                <div className="text-right">
                  <p className="text-xs text-muted-foreground">
                    {t("admin:dashboard.avgPerUser")}
                  </p>
                  <p className="text-sm font-bold text-foreground">
                    {data.wishlistAnalytics?.stats.avgWishlistSize.toFixed(1)}{" "}
                    {t("admin:pos.currentSale").toLowerCase()}
                  </p>
                </div>
              </div>
            </div>
                  ...
                  <tr className="text-[11px] uppercase tracking-wider text-muted-foreground font-semibold border-b border-border">
                    ...
                    <th className="px-6 py-4 text-center">
                      <div className="flex items-center justify-center gap-1">
                        ...
                        <span>{t("admin:dashboard.wishlistCount")}</span>
                      </div>
                    </th>
                    <th className="px-6 py-4 text-center">
