...
export function mapCartErrorMessage(
  ...
  if (message.includes('product not found') || message.includes('product does not exist')) {
    return 'This product is no longer available';
  }

  // Product availability errors
  if (message.includes('product is not available') || message.includes('product is unavailable')) {
  ...
  if (message.includes('cart not found') || message.includes('cart does not exist')) {
  ...
  if (message.includes('invalid coupon') || message.includes('coupon not found') || message.includes('coupon does not exist')) {
    return 'This coupon code is invalid';
  }

  if (message.includes('coupon expired') || message.includes('coupon has expired')) {
    return 'This coupon has expired';
  }

  if (message.includes('coupon not applicable') || message.includes('minimum order')) {
  ...
  if (message.includes('unauthorized') || message.includes('not authenticated') || message.includes('not logged in')) {
    return 'Please log in to manage your cart';
  }

  // Server errors (5xx)
  if (message.includes('server error') || message.includes('internal error') || message.match(/5\d{2}/)) {
 ...
 */
export function extractAvailableStock(error: any): number | undefined {
